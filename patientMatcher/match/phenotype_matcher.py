...
def match(database, max_score, features, disorders):
    ...
    if len(query_fields) > 0:
        ...
        LOG.info(
            "Searching for patients in database with the following query:{}".format(
                query
            )
        )
        ...
        for i in range(len(pheno_matching_patients)):
            ...
            LOG.info(
                "## Evaluating phenotype similarity with patient {} ##".format(i + 1)
            )
...
def evaluate_pheno_similariy(
    ...
    if hpo_terms and matching_hpo_terms:
        ...
        hpo_score = similarity_wrapper(
            hpoic, hpo, max_hpo_score, hpo_terms, matching_hpo_terms
        )
    ...
    if max_omim_score:  # OMIM terms can be compared
        omim_score = evaluate_subcategories(
            disorders, matching_omim_terms, max_omim_score
        )
...
def similarity_wrapper(hpoic, hpo, max_hpo_score, hpo_terms_q, hpo_terms_m):
    ...
    LOG.info(
        "patient-similarity module returned a simgic score of {}".format(simgic_score)
    )
