...
def add():
    ...
    if isinstance(
        formatted_patient, int
    ):  # some error must have occurred during validation
    ...
    if (
        ...
        and matching_obj
        and len(matching_obj.get("results"))
    ):
...
def delete(patient_id):
    ...
    resp = None
    if not authorize(
        current_app.db, request
    ):  # not authorized, return a 401 status code
...
def metrics():
    ...
    resp = jsonify(
        {"metrics": results, "disclaimer": current_app.config.get("DISCLAIMER")}
    )
    resp.status_code = 200
    return resp
...
def matches(patient_id):
    ...
    if results:
        ...
        message[
            "message"
        ] = "Could not find any matches in database for patient ID {}".format(
            patient_id
        )
    resp = jsonify(message)
    resp.status_code = 200
...
def match_external(patient_id):
    ...
    if not query_patient:
        LOG.error(
            "ERROR. Could not find any patient with ID {} in database".format(
                patient_id
            )
        )
        message[
    ...
    if node and not current_app.db["nodes"].find({"_id": node}).count():
        LOG.info(
            'ERROR, theres no node with id "{}" in database'.format(
                request.args["node"]
            )
        )
        message[
...
def match_internal():
    ...
    try:
        ...
        LOG.info(
            "An error has occurred while executing an internal patient match:{}".format(
                ex
            )
        )
