...
def check_request(database, request):
    ...
    formatted_patient = mme_patient(
        json_patient=request_json["patient"], convert_to_ensembl=True
    )
...
def delete_patient(database, patient_id):
    ...
    if deleted == 1:
        message[
            "message"
        ] = "Patient and its matches were successfully deleted from database"
    else:
        message[
            "message"
        ] = "ERROR. Could not delete a patient with ID {} from database".format(
            patient_id
        )
    return message
