...
def patient(id, label, remove_matches):
    ...
    n_removed = delete_by_query(
        query=query, mongo_db=current_app.db, mongo_collection="patients"
    )
    ...
    if remove_matches:
        ...
        n_removed = delete_by_query(
            query=query, mongo_db=current_app.db, mongo_collection="matches"
        )
        click.echo(
            "Number of matches for this patient removed from database:{}".format(
                n_removed
            )
        )
...
@click.option(
    ...
    nargs=1,
    required=True,
    help="ID of the client to be removed from database",
)
@with_appcontext
def client(id):
    ...
    query = {"_id": id}
    n_removed = delete_by_query(
        query=query, mongo_db=current_app.db, mongo_collection="clients"
    )
...
@click.option(
    ...
    nargs=1,
    required=True,
    help="ID of the node to be removed from database",
)
@with_appcontext
def node(id):
    ...
    query = {"_id": id}
    n_removed = delete_by_query(
        query=query, mongo_db=current_app.db, mongo_collection="nodes"
    )
