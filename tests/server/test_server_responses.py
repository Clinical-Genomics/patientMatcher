...
def test_match_async_request(
    mock_app, database, async_response_obj, json_patients, test_node
):
    ...
    data = {
        ...
        "source": "fakey node",
        "response": {
            "results": [{"score": {"patient": 0.8}, "patient": json_patients[1],}]
        },
    }
    ...
    assert (
        resp_data["message"]
        == "Error: could not create a valid match object from request data"
    )
    ...
    assert (
        mock_app.db["async_responses"]
        .find({"query_id": async_response_obj["query_id"]})
        .count()
        == 0
    )
...
def test_patient_matches(mock_app, database, match_objs, test_client):
    ...
    response = mock_app.test_client().get(
        "matches/unknown_patient", headers=auth_headers(ok_token)
    )
    ...
    response = mock_app.test_client().get(
        "matches/P0001058", headers=auth_headers(ok_token)
    )
    # response gives success
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_match_hgnc_symbol_patient(mock_app, gpx4_patients, test_client, database):
    ...
    assert (
        "contact" in data["results"][0]["patient"]
    )  # contact info should be available as well
...
def test_match_ensembl_patient(mock_app, test_client, gpx4_patients, database):
    ...
    assert query_patient["patient"]["genomicFeatures"][0]["gene"]["id"].startswith(
        "ENSG"
    )
...
def test_match_external(mock_app, test_client, test_node, database, json_patients):
    ...
    assert (
        data["message"]
        == "ERROR. Could not find any patient with ID not_a_valid_ID in database"
    )
    ...
    assert (
        data["message"]
        == "Could not find any other node connected to this MatchMaker server"
    )

    # Try to send a request for a match on a node that does not exist
    response = mock_app.test_client().post(
        "".join(["/match/external/", inserted_id, "?node=meh"]),
    ...
    assert (
        data["message"]
        == "ERROR. Could not find any connected node with id meh in database"
    )

    # insert a connected node
    add_node(
        mongo_db=mock_app.db, obj=test_node, is_client=False
    )  # required for external matches
