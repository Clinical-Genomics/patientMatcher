...
from patientMatcher.match.handler import (
    internal_matcher,
    save_async_response,
    external_matcher,
)
...
def test_internal_matching(database, gpx4_patients):
    ...
    assert (
        database["patients"]
        .find({"genomicFeatures.gene.id": "ENSG00000167468"})
        .count()
        == 2
    )
    ...
    assert (
        higest_scored_patient["score"]["patient"]
        > lowest_scored_patient["score"]["patient"]
    )
...
def test_external_matching(database, test_node, gpx4_patients, monkeypatch):
    ...
    inserted_ids = backend_add_patient(
        mongo_db=database, patient=patient, match_external=False
    )
    assert inserted_ids
