...
def test_add_client(database, test_client):
    ...
    inserted_id, collection = add_node(
        mongo_db=database, obj=test_client, is_client=is_client
    )
    ...
    inserted_id, collection = add_node(
        mongo_db=database, obj=test_client, is_client=True
    )
...
def test_add_server(database, test_node):
    ...
    inserted_id, collection = add_node(
        mongo_db=database, obj=test_node, is_client=is_client
    )
